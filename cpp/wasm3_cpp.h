...
namespace wasm3 {
    ...
    public:
        ...
        /**
         * Split compiler stage (support for RNWebAssembly).
         */
        void compile() {
    ...
    public:
        ...
        Ret call_argv(const std::vector<std::string>& args) {
            // Convert the vector of strings into an array of const char*
            ...
            /* std::enable_if above checks that all argument types are convertible const char* */
            M3Result res = m3_CallArgv(m_func, argv.size(), argv.data());

            detail::check_error(res);
            Ret ret;

            res = m3_GetResultsV(m_func, &ret);
         ...
         * @return the type of the argument at the specified index.
         */
        // HACK: RNWebAssembly
        M3ValueType GetArgType(const uint32_t index) {
          return m3_GetArgType(m_func, index);
        }

        /**
         * Get the number of arguments for the function.
         * @return the number of arguments in the function.
         */
        // HACK: RNWebAssembly
        uint32_t GetArgCount() {
          return m3_GetArgCount(m_func);
        }
         ...
         */
        // HACK: RNWebAssembly
        M3ValueType GetRetType(const uint32_t index) {
          return m3_GetRetType(m_func, index);
        }

        /**
         * Get the return count of a function.
         * @return the return count of the function.
         */
        // HACK: RNWebAssembly
        uint32_t GetRetCount() {
          return m3_GetRetCount(m_func);
        }
